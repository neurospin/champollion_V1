...
def get_region_list(crops_path):
    """Returns the list of regions in the given directory."""
    regions = []
    for root, dirs, files in os.walk(crops_path):
        for dir_name in dirs:
            if dir_name not in regions:
                regions.append(dir_name)
    return regions [:29]
...
def visualize_averages_along_sorted_phenotype(params,region=None, side=None,snapshot=False, save_dir=None):
    ...
    ### First and last group difference
    ...
    w_diff = a.createWindow('3D',block=_block)
    w_diff.addObjects(r_diff)
    r_diff.releaseAppRef()
    _average_dic['w_diff'] = w_diff


    ### Mid and last group difference
    mid_np = raw_avgs[3].np 

    # calculate difference
    mid_diff_np = late_np - mid_np

    # Reconstruction of an AIMS volum from diff_np

    size4d = raw_avgs[0].getSize()   # ex. (30,38,22,1)
    assert tuple(mid_diff_np.shape) == tuple(size4d)
    diff_vol2 = aims.Volume(size4d,"FLOAT") 


    diff_vol2.copyHeaderFrom(raw_avgs[0].header())             
    diff_vol2.np[:] = mid_diff_np                                

    # Conversion en AObject
    b_diff = a.toAObject(diff_vol2)

   
    # Volume‐rendering fusion
    r_diff2 = a.fusionObjects(
        objects=[b_diff],
        method='VolumeRenderingFusionMethod'
    )
   
    r_diff2.setPalette("diff-palette", minVal=-0.5, maxVal=0.5, absoluteMode=True)
    pal.releaseAppRef()   

    # 3D view for the difference
    w_diff2 = a.createWindow('3D',block=_block)
    w_diff2.addObjects(r_diff2)
    r_diff2.releaseAppRef
    _average_dic['w_diff2'] = w_diff2   
    ...
    if snapshot:
        ...
        for name, win in _average_dic.items():
            if name.startswith('wvr') or name.startswith('w_diff'):
                win.setHasCursor(0)
                fname = f"{region}_{name}.png"
                img_path = os.path.join(save_dir, fname)
                win.snapshot(img_path, width=1200, height=900)

                #print(f" Built snapshot {name} at {img_path}")

                # Close the window
                win.close()

         # Release all the AObject references
        for obj_name, obj in list(_average_dic.items()):
            try:
                obj.releaseAppRef()
            except Exception:
                pass
...
def create_grid(image_files, n_cols, out_path,title=None):
    ...
    grid = Image.new('RGB', (n_cols * w, n_rows * h + title_h), (255, 255, 255))
    # add the snnapshots to the grid
...
def main(argv):
    ...
    crop_path= f"/neurospin/dico/data/deep_folding/current/datasets/{dataset}/crops/2mm"
    save_path= f"/neurospin/dico/rmenasria/Runs/01_essai/Program/2023_jlaval_STSbabies/contrastive/notebooks/racim/Figures/Averages"

    if not os.path.exists(save_path):
        os.makedirs(save_path)


    regions_test= get_region_list(crop_path)
    regions = regions_test
    print(("regions", regions))
    sides = ['L', 'R']

    for side in sides: 
        print("Traitement de l'hémisphère", side)

        for region in regions:
            print("Treating region", region)
            save_dir = os.path.join(save_path, "blocks", side, region)
            os.makedirs(save_dir, exist_ok=True)

            # Compute averages along sorted phenotype
            visualize_averages_along_sorted_phenotype(params,region, side,snapshot=True, save_dir=save_dir)

            # Create a grid of the snapshots:
            pattern = os.path.join(save_dir, f"{region}_*.png")
            pngs = sorted(glob.glob(pattern))
            out_grid = os.path.join(save_dir, f"{region}_{side}_block.png")
            title = f"{region} — {side}"
            create_grid(pngs, n_cols=3, out_path=out_grid, title=title)
