...
print(f"{'='*30} Test R2 Summary {'='*30}")
print(f"{'Config':<45} {'Test R2':<20}")
print("-" * 70)
...
for config_name in sorted(os.listdir(root_dir)):
    ...
    if os.path.isdir(config_path):
        json_path = os.path.join(
            config_path,
            "ukb40_volume_random_embeddings",
            "volume",
            "test_values.json"
        )
        ...
        if os.path.exists(json_path):
            try:
                with open(json_path, "r") as f:
                    data = json.load(f)
                    test_r2 = data.get("test_r2", None)

                    if test_r2 is not None:
                        print(f"{config_name:<45} {test_r2:.4f}")
                    else:
                        print(f"{config_name:<45} Missing test_r2")
